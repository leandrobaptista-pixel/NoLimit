...
const routes = {
  ...
  requests: {
    ...
    heading: "Website visit requests",
    description: "Review new requests, open the customer details, and save the next step for your team.",
  },
  documents: {
    title: "Estimate & Contract",
    kicker: "Active client estimates",
    heading: "From site visit to approved contract without entering data twice.",
    description: "For active No Limit clients: build itemized estimates from registered project details, doors, windows, crown, dimensions, and individual or special prices. Once approved, generate the contract and invoice from the same information.",
  },
  clients: {
    title: "Clients",
...
function openDataEntry(type, recordId = "") {
  ...
  dataForm.reset();
  if (type === "person" && !recordId && location.hash === "#vendors") dataForm.elements.namedItem("type").value = "Vendor";
...
function renderRequests() { const records=filteredVisitRequests(), totals=Object.fromEntries(visitRequestCategories.map((category)=>[category,0])); visitIntake.items.filter((item)=>!item.discarded_at).forEach((item)=>{if(Object.prototype.hasOwnProperty.call(totals,item.project_type))totals[item.project_type]+=1;}); return `<section class="page requests-page">${pageHead(routes.requests,'<button class="button secondary" data-refresh-visits type="button">Refresh requests</button>')}<article class="panel visit-intake-overview"><div class="panel-head"><div><h2>Free On-Site Visit requests</h2><p>Private intake review. Opening contact does not change status; only an explicit save does.</p></div><span class="status-pill ${visitIntake.error?"red":"green"}">${escapeHtml(visitIntake.error||(visitIntake.loading?"Loading requests…":isLocalPreview?"Preview data":"Secure admin view"))}</span></div><div class="visit-filter-row" role="group" aria-label="Request filters"><button class="filter-chip${visitIntake.filter==="today"?" active":""}" data-visit-filter="today" type="button">Today</button><button class="filter-chip${visitIntake.filter==="pending"?" active":""}" data-visit-filter="pending" type="button">Pending</button><button class="filter-chip${visitIntake.filter==="all"?" active":""}" data-visit-filter="all" type="button">All</button><button class="filter-chip${visitIntake.showDiscarded?" active":""}" data-show-discarded type="button">${visitIntake.showDiscarded?"Back to active":"Discarded"}</button><label>Gallery section<select id="visitCategoryFilter"><option value="all">All 13 sections</option>${visitRequestCategories.map((category)=>`<option value="${escapeHtml(category)}"${visitIntake.category===category?" selected":""}>${escapeHtml(category)} (${totals[category]||0})</option>`).join("")}</select></label></div></article><details class="visit-category-overview"><summary>Requests by service category</summary><section class="visit-category-grid" aria-label="Request totals by gallery section">${visitRequestCategories.map((category)=>`<button class="visit-category-card${visitIntake.category===category?" active":""}" data-visit-category="${escapeHtml(category)}" type="button"><span>${escapeHtml(category)}</span><strong>${totals[category]||0}</strong></button>`).join("")}</section></details><section class="panel visit-intake-list"><div class="panel-head"><div><h2>${visitIntake.showDiscarded?"Discarded requests":"Requests to review"}</h2><p>${records.length} request${records.length===1?"":"s"} shown</p></div></div>${records.length?records.map(renderVisitRequestCard).join(""):visitIntake.loading?'<div class="empty-state" role="status"><h3>Loading requests…</h3><p>Reading the shared website records.</p></div>':'<div class="empty-state"><h3>No requests in this view</h3><p>Try another filter or refresh the secure intake.</p></div>'}</section></section>`; }
